...
public class ExampleTransformPlugin extends Transform<StructuredRecord, StructuredRecord> {
  ...
  public void transform(StructuredRecord input, Emitter<StructuredRecord> emitter) throws Exception {
    ...
    for(Schema.Field field : input.getSchema().getFields()) {
            ...
            String input_value = input.get(fieldName).toString() ;
            String output_value = Rot13.rotate(input_value);

         if((config.mask.contains(fieldName)))
            builder.set(fieldName, output_value);
          
         else{
            builder.set(fieldName, input_value);
...
//    private final Integer myOptionalOption;
    ...
    public Config(String mask, String schema) {
      this.mask = mask;
//      this.myOptionalOption = myOptionalOption;
    ...
    private void validate(Schema inputSchema) throws IllegalArgumentException {
      ...
      if (mask == null || mask.isEmpty()) {
        throw new IllegalArgumentException("mask is a required field.");
      }
